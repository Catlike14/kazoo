...
handle_member_message(JObj, Props, <<"connect_win">>) ->
    ...
    AgentProcessIds = kz_json:get_list_value(<<"Agent-Process-IDs">>, JObj, []),
    case lists:member(MyId, AgentProcessIds) of
...
handle_member_message(JObj, Props, <<"connect_satisfied">>) ->
    ...
    FSMPid = props:get_value('fsm_pid', Props),
    acdc_agent_fsm:member_connect_satisfied(FSMPid, JObj);
