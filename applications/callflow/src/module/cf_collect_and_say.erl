...
collect_more_digits(Data, Call, AlreadyCollected, MaxDigits) ->
    ...
    of
        {'ok', Ds} ->
            CollectionName = collection_name(Data),
            MediaId = media_id(Data),
            DTMFs = <<AlreadyCollected/binary, Ds/binary>>,
            lager:debug("collected ~s for ~s", [DTMFs, CollectionName]),
            _ = try
                _ = kapps_call_command:b_prompt(MediaId, Call),
                {'ok', _} = kapps_call_command:b_say(DTMFs, Call)
            catch
                _:_ -> 'ok'
            end,
            cf_exe:set_call(
...
-spec media_id(kz_json:object()) -> kz_term:ne_binary().
media_id(Data) ->
    kz_json:get_value(<<"media_id">>, Data).
